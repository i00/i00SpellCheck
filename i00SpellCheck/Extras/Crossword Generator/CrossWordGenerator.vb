...
#Region "Generate"
    ...
    Public Sub Generate(ByVal Dictionary As SpellCheckTextBox.Dictionary)
        Randomize()
        CrossWordCells.Clear()
        UsedWords = New HashSet(Of String)
        'initial word:
        'Put a random word on the board
        Dim Word = (From xItem In Dictionary Where xItem.ItemState <> SpellCheckTextBox.Dictionary.DictionaryItem.eItemState.Delete AndAlso xItem.Entry.Length <= Math.Max(CrossWordSize.Width, CrossWordSize.Height) And xItem.Entry.Length > 1 Order By Rnd() Select xItem.Entry).FirstOrDefault
    ...
    Private Sub AddAllWordsToGrid(ByVal Dictionary As SpellCheckTextBox.Dictionary)
        Randomize()
...
TryAgain:
    ...
    Private Function AddSingleWordAt(ByVal Dictionary As SpellCheckTextBox.Dictionary, ByVal Cell As CrossWordCell, ByVal ExtendDirection As ExtendDirections) As Boolean
...
NextPositionX:
    ...
    Private Function GetAllMatchingWords(ByVal Dictionary As SpellCheckTextBox.Dictionary, ByVal WordStartCell As Point, ByVal WordEndCell As Point, ByVal RequiredCell As CrossWordCell, Optional ByRef Pattern As String = "") As IEnumerable(Of String)
        Randomize()
        ...
        Dim Dict = (From xItem In Dictionary Where xItem.ItemState <> SpellCheckTextBox.Dictionary.DictionaryItem.eItemState.Delete AndAlso InStr(xItem.Entry, RequiredCell.Value, CompareMethod.Text) > 0 AndAlso xItem.Entry.Length < ((EndCellVariable - StartCellVariable) + 1) AndAlso xItem.Entry.Length > 1 AndAlso UsedWords.Contains(LCase(xItem.Entry)) = False Select xItem.Entry Order By Rnd()).ToArray
        Pattern = "^"
