...
#Region "Default and current dic"
    ...
    Public Shared Sub LoadDefaultDictionary()
        If SpellCheckTextBox.DefaultDictionary Is Nothing Then
            'load from file
            If FileIO.FileSystem.FileExists(DefaultDictFile) Then
                'load this
                Try
                    SpellCheckTextBox.DefaultDictionary = New Dictionary()
                    SpellCheckTextBox.DefaultDictionary.LoadFromFile(DefaultDictFile)
                Catch ex As Exception
                    'failed ... load blank one 
                    SpellCheckTextBox.DefaultDictionary = New Dictionary(DefaultDictFile, True)
                End Try
            Else
...
#End Region
    ...
    <System.ComponentModel.Editor(GetType(dicFile_UITypeEditor), GetType(System.Drawing.Design.UITypeEditor))> _
    ...
    Public Property CurrentDictionary() As Dictionary
        Get
            If mc_CurrentDictionary Is Nothing Then
                ...
                Return SpellCheckTextBox.DefaultDictionary
            Else
...
#Region "All SpellCheckTextBox Classes With the Same Dictionary"

    Public Sub InvalidateAllTextBoxesWithSameDict(Optional ByVal IncludeThis As Boolean = True)
        For Each iSellClassWithSameDict In (From xItem In AllSameDicCache() Where (IncludeThis OrElse xItem IsNot Me))
            'remove item from cache
            iSellClassWithSameDict.Invalidate()
        Next
    End Sub

    Friend Function AllSameDicCache() As IEnumerable(Of SpellCheckTextBox)
        Return (From xItem In SpellCheckTextBoxes Where xItem.Value.CurrentDictionary Is CurrentDictionary Select xItem.Value)
    End Function
